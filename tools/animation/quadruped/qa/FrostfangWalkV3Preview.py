...
SOURCE = ROOT / "Frostfang_WalkV3_IK_UNAPPROVED.blend"
...
def make_ground():
    ...
    ground.name = "DMMO_WalkV3_QA_Floor"
...
def render_views(scene):
    ...
    cameras = {
        "side": add_camera(
            "DMMO_WalkV3_Side", (2.4, -0.1, 0.75),
            (-0.09, -0.13, 0.49),
        ),
        "three_quarter": add_camera(
            "DMMO_WalkV3_Oblique", (2.0, -1.7, 1.05),
            (-0.09, -0.13, 0.49),
        ),
    }
    saved = []
    for view_name, camera in cameras.items():
        ...
        for frame in FRAMES:
            ...
            destination = PREVIEW / (
                f"Frostfang_WalkV3_{view_name}_{frame:03d}.png"
            )
