...
def main():
    ...
    destination = ROOT / "walk_v8_actual_mesh_audit.json"
    destination.write_text(
        json.dumps(report, indent=2), encoding="utf-8"
    )
    print("WALK_V8_MESH_SUMMARY", json.dumps(maxima), flush=True)
    print("WALK_V8_STANCE", json.dumps(stance), flush=True)
    print("WALK_V8_AUDIT_SAVED", destination, flush=True)
