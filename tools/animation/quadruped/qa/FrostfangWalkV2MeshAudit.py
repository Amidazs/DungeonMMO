"""Audit deformed Frostfang toe geometry during the walk-v2 trial.
...
ROOT = (
    ...
    "GroundedWalkTrial_20260923" / "WalkV2"
)
SOURCE = ROOT / "Frostfang_WalkV2_IK_UNAPPROVED.blend"
GROUP_NAMES = {
...
def main():
    ...
    manifest = json.loads(
        (ROOT / "walk_v2_manifest.json").read_text(encoding="utf-8")
    )
    groups = {
    ...
    destination = ROOT / "walk_v2_actual_mesh_audit.json"
    destination.write_text(
        json.dumps(report, indent=2), encoding="utf-8"
    )
    print("WALK_V2_MESH_SUMMARY", json.dumps(maxima), flush=True)
    print("WALK_V2_STANCE", json.dumps(stance), flush=True)
    print("WALK_V2_AUDIT_SAVED", destination, flush=True)
