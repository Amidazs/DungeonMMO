"""Audit deformed Frostfang toe geometry during the walk-v3 trial.
...
def main():
    ...
    manifest = json.loads(
        (ROOT / "walk_v3_manifest.json").read_text(encoding="utf-8")
    )
    groups = {
    ...
    destination = ROOT / "walk_v3_actual_mesh_audit.json"
    destination.write_text(
        json.dumps(report, indent=2), encoding="utf-8"
    )
    print("WALK_V3_MESH_SUMMARY", json.dumps(maxima), flush=True)
    print("WALK_V3_STANCE", json.dumps(stance), flush=True)
    print("WALK_V3_AUDIT_SAVED", destination, flush=True)
