...
def make_ground():
    ...
    ground.name = "DMMO_WalkV12_QA_Floor"
...
def render_views(scene):
    ...
    cameras = {
        "side": add_camera(
            "DMMO_WalkV12_Side", (2.4, -0.1, 0.75),
            (-0.09, -0.13, 0.49),
        ),
        "three_quarter": add_camera(
            "DMMO_WalkV12_Oblique", (2.0, -1.7, 1.05),
            (-0.09, -0.13, 0.49),
        ),
    }
    saved = []
    for view_name, camera in cameras.items():
        ...
        for frame in FRAMES:
            ...
            destination = PREVIEW / (
                f"Frostfang_WalkV12_{view_name}_{frame:03d}.png"
            )
            scene.render.filepath = str(destination)
            bpy.ops.render.render(write_still=True)
            saved.append(str(destination))
            print("WALK_V12_PREVIEW", destination, flush=True)
    return saved
...
def main():
    ...
    print("WALK_V12_PREVIEW_COMPLETE", len(images), flush=True)
