...
OUTPUT = ROOT / "GroundedWalkTrial_20260923" / "WalkV3"
FPS = 24
FINAL_FRAME = 72
WALK_START = 17
WALK_FINISH = 65
...
def prepare_rig():
    ...
    if mesh.parent is not None:
        raise RuntimeError("Expected an unparented experimental source mesh")
    # Keep the source mesh locally aligned with its skinning armature.
    # Otherwise translation animates the bones but not the full wolf mesh.
    mesh.parent = rig
...
def save_results(rig, scene, legs, record):
    ...
    blend_path = OUTPUT / "Frostfang_WalkV3_IK_UNAPPROVED.blend"
    metadata = {
    ...
    (OUTPUT / "walk_v3_manifest.json").write_text(
        json.dumps(metadata, indent=2), encoding="utf-8"
    )
    print("WALK_V3_SAVED", blend_path, flush=True)
